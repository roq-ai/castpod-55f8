...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createEpisode() {
    ...
    const body = { ...req.body };
    if (body?.playlist_episode?.length > 0) {
      const create_playlist_episode = body.playlist_episode;
      body.playlist_episode = {
        create: create_playlist_episode,
      };
    } else {
      delete body.playlist_episode;
    }
    if (body?.saved_episode?.length > 0) {
      const create_saved_episode = body.saved_episode;
      body.saved_episode = {
        create: create_saved_episode,
      };
    } else {
      delete body.saved_episode;
    }
